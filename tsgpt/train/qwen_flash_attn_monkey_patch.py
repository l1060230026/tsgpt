...
def forward(
    ...
    if output_attentions:
        warnings.warn(
            "Output attentions is not supported for patched `Qwen2Attention`, returning `None` instead."
        )
...
# Disable the transformation of the attention mask in Qwen2Model as flash attention
...
def replace_qwen_attn_with_flash_attn():
    ...
    Qwen2Model._prepare_decoder_attention_mask = _prepare_decoder_attention_mask
...
def test():
    ...
    attn = Qwen2Attention(config).to(device).half()
