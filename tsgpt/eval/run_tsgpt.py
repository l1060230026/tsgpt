...
def load_model(args, gpu_id):
    ...
    model = STQwen2ForCausalLM.from_pretrained(args.base_model, torch_dtype=torch.bfloat16, use_cache=True,
    ...
    # 使用 device 0，因为我们已经通过 CUDA_VISIBLE_DEVICES 设置了可见的 GPU
    model.cuda(gpu_id)
    return model, tokenizer

def worker_process(gpu_id, model, tokenizer, input_queue, output_queue, stop_event, batch_size=4, num_samples=3):
    while not stop_event.is_set():
        try:
            ...
            solutions = {}  # Change to dictionary
            ...
            for idx, instruct_item, st_dict in batch_data:
                st_data = st_dict['st_data']
                qs = st_dict["query"]
                qs += solutions.get(idx, '')  # Use get() with default empty string
...
def run_eval(args):
    ...
    for gpu_id in range(args.num_gpus):
        ...
        p = Process(target=worker_process, args=(gpu_id, model, tokenizer, input_queue, output_queue, stop_event, args.batch_size, args.num_samples))
        p.start()
        processes.append(p)
    ...
    while pbar.n < len(prompt_file):
        try:
            ...
            if result is not None:
                ...
                with open(osp.join(args.output_res_path, f'final_results.json'), "w") as fout:
